...
class BlockAttentionResiduals(nn.Module):
    def __init__(
        ...
        if isinstance(shortcut, (int, float)):
            shortcut = bool(int(shortcut))
        elif isinstance(shortcut, str):
            shortcut = shortcut.lower() in ('true', '1', 'yes')

        # 处理 g
        if isinstance(g, float):
            g = int(g)
        elif isinstance(g, str):
            g = int(g) if g.isdigit() else 1

        # 处理 e
        if isinstance(e, str):
            e = float(e)

        # 处理 num_attention_blocks
        if isinstance(num_attention_blocks, (int, float, str)):
            num_attention_blocks = int(float(num_attention_blocks))

        g = max(1, g)
        num_attention_blocks = max(1, min(num_attention_blocks, 6))

        self.num_attention_blocks = num_attention_blocks
        self.block_size = 2

        self.sub_blocks = nn.ModuleList([
            Conv(c1, c2, k=1 if c1 != c2 else 3, s=1),
            Conv(c2, c2, k=3, s=1),
            Bottleneck(c2, c2, shortcut, g, e=1.0),
            Bottleneck(c2, c2, shortcut, g, e=1.0),
            Bottleneck(c2, c2, shortcut, g, e=1.0),
            Conv(c2, c2, k=3, s=1),
        ])
        ...
        # 注意力组件
        self.token_embed = nn.Parameter(torch.randn(1, c2, 1, 1) * 0.02)
        self.pseudo_queries = nn.ParameterList([
            nn.Parameter(torch.zeros(1, c2, 1, 1)) for _ in range(num_attention_blocks)
        ])
        self.rms_norm = RMSNorm2D(c2)
        self.value_proj = nn.Conv2d(c2, c2, 1, bias=False)

        # 初始化权重
        self._init_weights()

    def _init_weights(self):
        """论文Section 5中提到所有伪查询向量必须初始化为零"""
        for q in self.pseudo_queries:
            nn.init.zeros_(q)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape

        # 历史块列表，初始为 token embedding b₀
        blocks = [self.token_embed.expand(B, -1, H, W)]
        partial = None
        attn_idx = 0

        for i, block in enumerate(self.sub_blocks):
            # 块边界：完成当前块，加入历史
            if i % self.block_size == 0 and i > 0:
                blocks.append(partial)
                partial = None
                attn_idx = min(attn_idx + 1, self.num_attention_blocks - 1)

            # 计算输入（第一层用原始 x，否则用注意力聚合）
            if i == 0:
                h = x
            else:
                sources = blocks + ([partial] if partial is not None else [])
                h = self._attend(self.pseudo_queries[attn_idx], torch.stack(sources, dim=0))
        ...
        # 最终输出：聚合所有历史块
        blocks.append(partial)
        output = self._attend(self.pseudo_queries[-1], torch.stack(blocks[1:], dim=0))

        return output

    def _attend(self, query: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
        """注意力聚合：query 对 values 做 softmax 注意力"""
        N, B, C, H, W = values.shape

        # 归一化 keys
        keys_norm = self.rms_norm(values.view(-1, C, H, W)).view(N, B, C, H, W)
        q = query.view(C)

        # 计算注意力权重
        logits = torch.einsum('c, n b c h w -> n b h w', q, keys_norm)
        weights = torch.softmax(logits, dim=0).unsqueeze(2)

        # 加权求和并投影
        return self.value_proj(torch.sum(weights * values, dim=0))
...
#         if isinstance(e, str):
#             e = float(e)
#         if isinstance(num_attention_blocks, float):
#             num_attention_blocks = int(num_attention_blocks)
#         elif isinstance(num_attention_blocks, str):
#             num_attention_blocks = int(num_attention_blocks)
#         # if isinstance(dropout, (int, float, str)):
