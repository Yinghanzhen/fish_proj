...
# 从同目录的 multi_task_yolo.py 导入网络与 Loss
from multi_task_yolo import MultiTaskYOLO, MultiTaskYOLOLoss


# 1. 读取 AnyLabeling JSON 数据集类
...
def train():
    # ------------------ 参数配置区 ------------------
    DATA_DIR = "./dataset/annotations/"  # 存放 jpg 和 json 的文件夹
    KPT_NAMES = ["head", "tail"]  # 关键点名称
    VAL_RATIO = 0.2  # 验证集占比 (8:2)
    ...
    best_val_loss = float("inf")  # 用于保存最佳权重的基准线
    ...
    # 保存最后一轮权重作为最后一个 checkpoint
    torch.save(model.state_dict(), "fish_multitask_last.pth")
    print("训练全部结束！最后一轮权重已另存为: fish_multitask_last.pth")
